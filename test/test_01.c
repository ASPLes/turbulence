...
/* ------------------------------------------------------------------ */
/* mod-sasl-mysql: configuration layer and DTD (test 12-d / 12-e)
 *
 * WHY THESE EXIST
 * ---------------
 * Every defect the mysql sasl backend has had showed up only at
 * runtime, on a live authentication. On 2026-08-17 that cost a
 * production panel every single login: an automated tool rewrote
 * auth-db.mysql.xml through an XML serializer, the SQL apostrophes
 * became &apos;, <get-password> and <ip-filter> were read without
 * translating entities, and MySQL answered 1064 to every statement.
 *
 * The configuration layer was split out (mod_sasl_mysql_conf.c) so it
 * depends on libaxl alone: reading a template, deciding which
 * declarations apply and substituting tokens are pure operations that
 * can be pinned here. Escaping is injected as a handler, so the
 * substitution logic runs exactly as it does in production while the
 * test supplies a deterministic escaper.
 * ------------------------------------------------------------------ */

/* bookkeeping: these tests assert many small properties, so they
 * accumulate failures and report them all instead of stopping at the
 * first one. The test returns axl_false when any of them failed. */
int mysql_conf_failures = 0;

void mysql_conf_check (axl_bool condition, const char * description)
{
	if (condition)
		return;
	printf ("ERROR: %s\n", description);
...
void mysql_conf_check_str (const char * got, const char * expected, const char * description)
{
...
/* Escape handler used by these tests. It wraps the value in <> so a
 * test can tell, unambiguously, which parts of a statement went through
 * escaping and which did not. That is the property that matters: %t
 * must NOT be escaped (it is a constant produced by the backend) while
 * every peer provided token MUST be. */
char * mysql_conf_escape (axlPointer data, const char * value)
{
	if (value == NULL)
		return NULL;
...
char * mysql_conf_escape_null (axlPointer data, const char * value)
{
	return NULL;
}


/* Builds a minimum auth-db document whose root carries the provided
 * attribute declaration (empty string for none) and reports what
 * mod_sasl_mysql_resolve_sets_auth_id decides for it. */
void mysql_conf_check_set_auth_id (const char * root_attrs, axl_bool expected, const char * description)
{
	axlDoc   * doc;
	axlError * err = NULL;
	char     * content;
	...
	if (doc == NULL) {
		printf ("ERROR: %s (document did not parse: %s)\n", description,
	...
	mysql_conf_check (mod_sasl_mysql_resolve_sets_auth_id (doc) == expected, description);
	axl_doc_free (doc);
	return;
}

/* Validates a complete auth-db document against the compiled DTD. */
void mysql_conf_check_dtd (axlDtd * dtd, const char * content, axl_bool should_validate, const char * description)
{
	axlDoc   * doc;
	axlError * err = NULL;
	axl_bool   valid;
	...
	doc = axl_doc_parse (content, -1, &err);
	if (doc == NULL) {
		printf ("ERROR: %s (document did not even parse: %s)\n", description,
			err ? axl_error_get (err) : "<no error>");
		if (err)
			axl_error_free (err);
		mysql_conf_failures++;
	...
	mysql_conf_check (valid == should_validate, description);
	return;
}

/* Same, for a body inserted into the minimum valid document. */
void mysql_conf_check_dtd_body (axlDtd * dtd, const char * body, axl_bool should_validate, const char * description)
{
	char * content;
	...
	content = axl_strdup_printf (
		...
		"<get-password query=\"SELECT password FROM user WHERE name like binary '%%u'\" />"
		"%s</sasl-auth-db>", body);
	mysql_conf_check_dtd (dtd, content, should_validate, description);
	axl_free (content);
	return;
}
 ...
 * (password replaced). */
#define MYSQL_CONF_DEPLOYED_FILE \
	"<?xml version='1.0' ?>\n" \
	"<sasl-auth-db>\n" \
	"        <!--  connection settings  -->\n" \
...
/**
 * @brief Test 12-d: configuration layer of the mysql sasl backend:
 * reading SQL templates (XML entity translation), token substitution
 * and which declarations apply.
 *
 * Needs no database: it exercises mod_sasl_mysql_conf.c directly.
 */
axl_bool test_12d (void) {
	axlDoc            * doc;
	axlNode           * node;
	axlError          * err = NULL;
	char              * query;
	...
	mysql_conf_check_str (query, "SELECT p FROM u WHERE n = '%u'",
			      "&apos; is translated into a real apostrophe (2026-08-17 outage)");
	if (query)
		axl_free (query);
	axl_doc_free (doc);

	/* the shape core-admin ships: double quoted attribute with literal
	 * apostrophes. Must come out untouched */
	doc = axl_doc_parse ("<root><get-password query=\"SELECT p FROM u WHERE n = '%u'\" /></root>", -1, &err);
	mysql_conf_check (doc != NULL, "document with literal apostrophes parses");
	if (doc == NULL) {
		if (err)
			axl_error_free (err);
		return axl_false;
	...
	node  = axl_doc_get (doc, "/root/get-password");
	query = mod_sasl_mysql_get_query (node);
	mysql_conf_check_str (query, "SELECT p FROM u WHERE n = '%u'", "literal apostrophes are preserved");
	if (query)
		axl_free (query);
	...
	query = mod_sasl_mysql_get_query (node);
	mysql_conf_check (query == NULL, "node without query attribute reports NULL");
	mysql_conf_check (mod_sasl_mysql_get_query (NULL) == NULL, "NULL node reports NULL");
	axl_doc_free (doc);
	...
	if (doc != NULL) {
		...
		query = mod_sasl_mysql_get_query (node);
		mysql_conf_check_str (query, "SELECT 1 WHERE a < b AND c > d AND e = \"x\" AND f & g",
	...
	query = mod_sasl_mysql_build_query ("u=%u n=%n i=%i m=%m p=%p t=%t e=%e", &subst, mysql_conf_escape, NULL);
	mysql_conf_check_str (query,
	...
	query = mod_sasl_mysql_build_query ("%u and %u again", &subst, mysql_conf_escape, NULL);
	mysql_conf_check_str (query, "<alice> and <alice> again", "repeated token replaced everywhere");
	if (query)
		axl_free (query);

	/* NULL values become the empty string, never a dangling token: an
	 * unresolved %e reaching MySQL would be a syntax error */
	subst.status       = NULL;
	subst.effective_id = NULL;
	query = mod_sasl_mysql_build_query ("t=[%t] e=[%e]", &subst, mysql_conf_escape, NULL);
	mysql_conf_check_str (query, "t=[] e=[]", "NULL status and identity become empty, not a dangling token");
	if (query)
		axl_free (query);

	/* an escaper that fails must not leave the token behind either */
	subst.status = "failed";
	query = mod_sasl_mysql_build_query ("u=[%u] t=[%t]", &subst, mysql_conf_escape_null, NULL);
	mysql_conf_check_str (query, "u=[] t=[failed]", "failing escaper yields empty value, never the raw token");
	if (query)
		axl_free (query);

	query = mod_sasl_mysql_build_query ("SELECT 1", &subst, mysql_conf_escape, NULL);
	mysql_conf_check_str (query, "SELECT 1", "template without tokens is unchanged");
	if (query)
		axl_free (query);

	mysql_conf_check (mod_sasl_mysql_build_query (NULL, &subst, mysql_conf_escape, NULL) == NULL,
			  "NULL template reports NULL");
	mysql_conf_check (mod_sasl_mysql_build_query ("x", NULL, mysql_conf_escape, NULL) == NULL,
			  "NULL substitution set reports NULL");
	mysql_conf_check (mod_sasl_mysql_build_query ("x", &subst, NULL, NULL) == NULL,
			  "NULL escaper reports NULL");
	...
	doc = axl_doc_parse (
		...
		"</root>", -1, &err);
	mysql_conf_check (doc != NULL, "declarations document parses");
	if (doc == NULL) {
		if (err)
			axl_error_free (err);
		return axl_false;
	...
	mysql_conf_check_str (mod_sasl_mysql_filter_stage (node), "pre-auth", "filter stage defaults to pre-auth");
	mysql_conf_check_str (mod_sasl_mysql_filter_match (node), "expression", "filter match defaults to expression");
	mysql_conf_check_str (mod_sasl_mysql_node_name (node), "unnamed", "name defaults to unnamed");
	mysql_conf_check (mod_sasl_mysql_filter_applies (node, "pre-auth"), "default filter applies to pre-auth");
	mysql_conf_check (! mod_sasl_mysql_filter_applies (node, "post-auth"), "default filter does not apply to post-auth");

	node = axl_node_get_next_called (node, "auth-filter");
	mysql_conf_check_str (mod_sasl_mysql_filter_stage (node), "post-auth", "declared stage is honoured");
	mysql_conf_check_str (mod_sasl_mysql_filter_match (node), "required", "declared match is honoured");
	mysql_conf_check_str (mod_sasl_mysql_node_name (node), "named", "declared name is honoured");
	mysql_conf_check (mod_sasl_mysql_filter_applies (node, "post-auth"), "post-auth filter applies to post-auth");
	mysql_conf_check (! mod_sasl_mysql_filter_applies (node, "pre-auth"), "post-auth filter does not apply to pre-auth");

	/* an unknown match must be rejected, never treated as "no
	 * restriction": a typo would silently disable the filter */
	node = axl_node_get_next_called (node, "auth-filter");
	mysql_conf_check (! mod_sasl_mysql_match_is_known (mod_sasl_mysql_filter_match (node)), "unknown match mode is rejected");
	mysql_conf_check (mod_sasl_mysql_match_is_known ("expression"), "expression is a known match mode");
	mysql_conf_check (mod_sasl_mysql_match_is_known ("required"), "required is a known match mode");
	mysql_conf_check (mod_sasl_mysql_match_is_known ("forbidden"), "forbidden is a known match mode");
	mysql_conf_check (! mod_sasl_mysql_match_is_known (NULL), "NULL match mode is rejected");

	node = axl_doc_get (doc, "/root/auth-notify");
	mysql_conf_check_str (mod_sasl_mysql_notify_on (node), "any", "notify selector defaults to any");
	mysql_conf_check (mod_sasl_mysql_notify_applies (node, "ok"), "default notify fires on ok");
	mysql_conf_check (mod_sasl_mysql_notify_applies (node, "failed"), "default notify fires on failed");
	mysql_conf_check (mod_sasl_mysql_notify_applies (node, NULL), "default notify fires with unknown status");

	node = axl_node_get_next_called (node, "auth-notify");
	mysql_conf_check (mod_sasl_mysql_notify_applies (node, "ok"), "on=ok fires on ok");
	mysql_conf_check (! mod_sasl_mysql_notify_applies (node, "failed"), "on=ok does not fire on failed");
	mysql_conf_check (! mod_sasl_mysql_notify_applies (node, NULL), "on=ok does not fire with unknown status");

	node = axl_node_get_next_called (node, "auth-notify");
	mysql_conf_check (mod_sasl_mysql_notify_applies (node, "failed"), "on=failed fires on failed");
	mysql_conf_check (! mod_sasl_mysql_notify_applies (node, "ok"), "on=failed does not fire on ok");

	mysql_conf_check (! mod_sasl_mysql_filter_applies (NULL, "pre-auth"), "NULL filter node does not apply");
	mysql_conf_check (! mod_sasl_mysql_notify_applies (NULL, "ok"), "NULL notify node does not apply");

	axl_doc_free (doc);

	/* ---- 4. publishing a resolved identity as the SASL auth id ---- */
	 ...
	 * an api-token style deployment needs, and a silent "no" would
	 * leave the session running under the credential instead of the
	 * identity it resolved onto. */
	mysql_conf_check_set_auth_id ("", axl_true, "not declared defaults to publishing");
	mysql_conf_check_set_auth_id ("set-auth-id='yes'", axl_true, "set-auth-id=yes publishes");
	mysql_conf_check_set_auth_id ("set-auth-id='no'", axl_false, "set-auth-id=no does not publish");

	/* Only the exact value disables it. Anything else keeps the default
	 * rather than silently changing which identity a session runs as
	 * because of a typo. */
	mysql_conf_check_set_auth_id ("set-auth-id='NO'", axl_true, "set-auth-id=NO is not the opt out (exact match only)");
	mysql_conf_check_set_auth_id ("set-auth-id='false'", axl_true, "set-auth-id=false keeps the default");
	mysql_conf_check_set_auth_id ("set-auth-id='0'", axl_true, "set-auth-id=0 keeps the default");
	mysql_conf_check_set_auth_id ("set-auth-id=''", axl_true, "empty set-auth-id keeps the default");
	mysql_conf_check (mod_sasl_mysql_resolve_sets_auth_id (NULL) == axl_true, "NULL document defaults to publishing");

	if (mysql_conf_failures) {
		printf ("ERROR: %d configuration layer checks failed\n", mysql_conf_failures);
		return axl_false;
	} /* end if */
...
/**
 * @brief Test 12-e: the DTD that validates auth-db.mysql.xml.
 *
 * Two properties matter here. First, that the extension points are
 * accepted and malformed declarations refused. Second, and more
 * important, BACKWARD COMPATIBILITY: updating turbulence on a
 * core-admin server replaces the DTD compiled into the module, and
 * mod-sasl refuses to load a file that does not validate. A DTD that
 * stopped accepting a shape already deployed would lock every user out
 * of that panel the moment the package lands -- the 2026-08-17 failure
 * reached from the other side.
 *
 * On paper that cannot happen: every content model this DTD ever had is
 * a prefix of the current one, so the accepted language only ever grew,
 * and a file loading today validates against its own older DTD by
 * definition. These cases pin that reasoning against the real shapes
 * instead of trusting it.
 *
 * Needs no database.
 */
axl_bool test_12e (void) {
	axlDtd   * dtd;
	axlError * err = NULL;
	...
	mysql_conf_check (dtd != NULL, "the compiled DTD parses");
	if (dtd == NULL) {
		if (err)
			axl_error_free (err);
		return axl_false;
	...
	/* ---- what a file is allowed to declare ---- */

	mysql_conf_check_dtd_body (dtd, "", axl_true, "minimum file (connection-settings + get-password) validates");
	mysql_conf_check_dtd_body (dtd, "<auth-filter query='q' />", axl_true, "a single auth-filter validates");
	mysql_conf_check_dtd_body (dtd, "<auth-filter query='q' /><auth-filter query='q' stage='post-auth' match='required' name='x' />",
				   axl_true, "repeated auth-filter validates");
	mysql_conf_check_dtd_body (dtd, "<auth-resolve query='q' /><auth-resolve query='q' name='x' />",
				   axl_true, "repeated auth-resolve validates");
	mysql_conf_check_dtd_body (dtd, "<auth-notify query='q' /><auth-notify query='q' on='ok' name='x' />",
	...
	mysql_conf_check_dtd_body (dtd, "<not-a-known-node query='q' />", axl_false, "unknown declaration is refused");

	/* the root attribute controlling identity publication */
	mysql_conf_check_dtd (dtd,
			      "<sasl-auth-db set-auth-id='no'>"
			      "<connection-settings port='' host='localhost' database='d' password='p' user='u' />"
			      "<get-password query='q' /></sasl-auth-db>",
			      axl_true, "root set-auth-id=no validates");
	mysql_conf_check_dtd (dtd,
			      "<sasl-auth-db set-auth-id='yes'>"
			      "<connection-settings port='' host='localhost' database='d' password='p' user='u' />"
			      "<get-password query='q' /></sasl-auth-db>",
			      axl_true, "root set-auth-id=yes validates");
	mysql_conf_check_dtd (dtd,
			      "<sasl-auth-db not-a-known-attr='x'>"
			      "<connection-settings port='' host='localhost' database='d' password='p' user='u' />"
			      "<get-password query='q' /></sasl-auth-db>",
			      axl_false, "unknown root attribute is refused");

	/* ---- BACKWARD COMPATIBILITY with what is already deployed ---- */

	mysql_conf_check_dtd (dtd, MYSQL_CONF_DEPLOYED_FILE, axl_true,
			      "the file deployed on a production server still validates");

	/* the shape of the oldest installations */
	mysql_conf_check_dtd (dtd,
			      "<sasl-auth-db>"
			      "<connection-settings port='' host='localhost' database='d' password='p' user='u' />"
	...
	/* after <auth-log>/<ip-filter> were introduced, before the -alt
	 * nodes existed: a server that never ran core-admin's micro-update 001 */
	mysql_conf_check_dtd (dtd,
			      "<sasl-auth-db>"
			      "<connection-settings port='' host='localhost' database='d' password='p' user='u' />"
			      "<get-password query=\"SELECT password FROM user WHERE name = '%u'\" />"
	...
	mysql_conf_check_dtd_body (dtd, "<auth-log query='q' />", axl_true, "auth-log without ip-filter validates");
	mysql_conf_check_dtd_body (dtd, "<ip-filter query='q' />", axl_true, "ip-filter without auth-log validates");

	/* a server left in the state core-admin's micro-update 036
	 * produced, with XML entities inside the queries: it must still
	 * LOAD. The statements are wrong, but refusing the document would
	 * take the panel down at package install time instead of letting
	 * the recovery run. */
	mysql_conf_check_dtd (dtd,
			      "<sasl-auth-db>"
			      "<connection-settings port='' host='localhost' database='d' password='p' user='u' />"
	 ...
	 * produces these; they are here so a future reordering is noticed. */
	mysql_conf_check_dtd (dtd,
			      "<sasl-auth-db>"
	...
	mysql_conf_check_dtd_body (dtd, "<ip-filter query='q' /><auth-log query='q' />", axl_false,
				   "ip-filter before auth-log is refused");
	mysql_conf_check_dtd (dtd,
			      "<sasl-auth-db>"
			      "<connection-settings port='' host='localhost' database='d' password='p' user='u' />"
			      "</sasl-auth-db>",
	...
	if (mysql_conf_failures) {
		printf ("ERROR: %d DTD checks failed\n", mysql_conf_failures);
		return axl_false;
	} /* end if */
...
int main (int argc, char ** argv)
{
	...
	CHECK_TEST("test_12d")
	run_test (test_12d, "Test 12-d: mod sasl mysql (configuration layer: entities, substitution, predicates)");

	CHECK_TEST("test_12e")
	run_test (test_12e, "Test 12-e: mod sasl mysql (DTD and backward compatibility of auth-db.mysql.xml)");
